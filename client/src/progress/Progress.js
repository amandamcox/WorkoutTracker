...
const Progress = () => {
	...
	const availableExercises = useSelector(
		state => state.tracker.uniqueUserExercises
	)
	const isLoggedIn = useSelector(state => state.auth.isLoggedIn)
	const initialData = useSelector(state => state.tracker.userWorkouts)

	const dispatch = useDispatch()
	const userToken = JSON.parse(window.localStorage.getItem('workoutTracker'))
		.token
	...
	const filterData = exercise => {
		...
		return dataCopy.filter(workout => {
			if (workout.exercise === exercise) return workout
		})
	}
